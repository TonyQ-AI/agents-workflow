# Reasonix Workflow Plugin

> 多 Agent 协同开发工作流插件包 — 用于在其他 AI 编码工具（Cursor、Claude Code、Codex、OpenCode 等）中安装  
> 版本：4.1 · 仓库：https://github.com/TonyQ-AI/workflow-plugin
...
> | 仓库 | 用途 |
> |------|------|
> | [`reasonix-workflow`](https://github.com/TonyQ-AI/reasonix-workflow) | **工作流本体** — 安装到 Reasonix 桌面端使用，直接引用 `skills/` 目录 |
> | [`workflow-plugin`](https://github.com/TonyQ-AI/workflow-plugin)（本仓库） | **插件发布包** — 供 Cursor、Claude Code、Codex、OpenCode 等工具通过 `install_source` 或 `.mcp.json` 安装 |
>
> **两个仓库的技能代码完全一致**（40 个技能 SKILL.md 内容相同），只是包装形式不同。  
> 开发在 [`reasonix-workflow`](https://github.com/TonyQ-AI/reasonix-workflow) 进行，本仓库定期同步。
