...
AI 会自动克隆本仓库（https://github.com/TonyQ-AI/reasonix-workflow）并完成全部安装。

或者直接说 `/Reasonix Workflow-workflow` 也能触发安装。
...
在 `Reasonix Workflow.toml` 中添加：

```toml
[skills]
paths = ["Reasonix Workflow-workflow/skills"]
```
...
重启 Reasonix Workflow 使新技能和 MCP 配置生效。
...
| 组件 | 要求 |
|------|------|
| Reasonix Workflow | 桌面版（支持 task 子Agent） |
...
### wf-orchestrator 没反应
- 确认 .Reasonix Workflow/skills/ 下有 `wf-orchestrator` 和 `wf-orchestrator-engine`
- 确认 AGENTS.md 存在
...
- 插件包（其他工具用）：https://github.com/TonyQ-AI/reasonix-workflow-plugin
