...
在需求澄清过程中，对话中出现的**业务术语**不要等文档写完再记——**当时就写**到 CONTEXT.md。

**触发规则：**
- 用户说了一个业务概念并给出定义 → 立即追加到 CONTEXT.md
- 发现两个术语是同义词 → 记录禁用别称
...
→ 没有"播放列表"这个词 → 追加：
  ## 通用语言
  - 播放列表（Playlist）：视频的有序集合，每个用户可拥有多个
  - 一个用户可有多个播放列表（1:N 关系）
```

这样 brainstorming 结束后的产出不只是规格文档，还有一份已经累积了术语的 CONTEXT.md，后面的 domain-modeling 可以直接在此基础上深化。
