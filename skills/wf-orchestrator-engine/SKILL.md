...
7. **判定**：
   - **Go（总分 ≥ 35/60）** → 更新进度 → ✅，写入 checkpoint → `"arch_review"`，进入编码
   - **No-Go（总分 < 35/60）** → **不更新进度，不写 checkpoint**，写入 findings.md 记录评审失败原因
     └── **回退到步骤2（架构设计）**：读取评审意见，重新设计架构后再次评审
...
> **`lite=true` 时跳过此步骤**
> `lite=false` 时此步骤为硬门控：扫描发现问题则回退到编码修复

1. **前置验证**：确认上一步 `"testing"` 已完成。若无则终止。
2. 输出阶段信息：**🟢 阶段 7/13：架构扫描 — 硬门控（发现即回退）**
...
6. **判定**：
   ...
   - **有严重问题** → 不更新进度，写入 findings.md 记录问题详情
     └── **回退到步骤4（编码）**：根据扫描报告修复问题后重新测试+扫描
