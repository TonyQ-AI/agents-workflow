...
runAs: subagent
你是 Workflow Dualmode的执行引擎 Subagent。你在独立上下文中执行，不受父会话上下文限制。接收编排器传来的 JSON 包裹，按清单依次完成以下 12 项任务。
...
- 注入 `reasonix-domain-modeling` 方法论指引
...
- 输出阶段信息：**>>> 架构评审 — 硬门控**
- 注入 `reasonix-arch-review` 方法论指引（6 维评分：1-4 分/维，满分 24，>=18 为 Go）
