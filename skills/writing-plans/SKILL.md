...
### 1. `task_plan.md` — 任务清单（带勾选框）

从计划中提取所有任务和步骤，生成可勾选的清单：
...
- **writing-plans** 负责首次创建这三个文件（所有任务显示为未勾选状态）
- **executing-plans** 在执行过程中逐项勾选、更新进度、记录发现
- **wf-orchestrator** 在派发子Agent前读取这三个文件，注入进度信息到 prompt
- **AGENTS.md** 中包含兜底规则：任何会话开始前自动检测这些文件
