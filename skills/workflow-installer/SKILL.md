...
当用户说「装工作流」、`安装 workflow 工作流`、`/Workflow-Dualmode` 时，按以下步骤执行。
...
```powershell
# 克隆到临时目录
git clone --depth 1 https://github.com/TonyQ-AI/Workflow-Dualmode.git $env:TEMP/Workflow-Dualmode

# 如果 git clone 失败（网络问题），尝试下载 ZIP：
# Invoke-WebRequest -Uri "https://github.com/TonyQ-AI/Workflow-Dualmode/archive/refs/heads/main.zip" -OutFile $env:TEMP/workflow.zip
# Expand-Archive $env:TEMP/workflow.zip $env:TEMP/Workflow-Dualmode
```
...
Copy-Item -Recurse "$env:TEMP/Workflow-Dualmode/skills/*" ".workflow/skills/" -Force
```
...
详细文档：
  https://github.com/TonyQ-AI/Workflow-Dualmode
```
