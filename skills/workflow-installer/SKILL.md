...
**第〇步 — 判断是否需要 MiMo：**
检查当前使用的推理模型是否原生支持多模态（图片/音频/视频理解）：

▸ 原生支持多模态的模型（GPT-4o、Claude 3.5、Gemini 等）：
> ℹ️ 你使用的模型已原生支持多模态，无需 MiMo 即可处理图片/音频/视频。
> 是否仍要安装 MiMo？（如果你以后可能切换到 DeepSeek 等不支持多模态的模型，建议安装）
> - 安装 → 继续第一步
> - 跳过 → 不配置 MiMo，跳到步骤5

▸ 不支持多模态的模型（DeepSeek、Qwen 纯文本版等）：
> ⚠️ 你使用的模型不支持图片/音频/视频理解。
> 强烈建议安装 MiMo 补充多模态能力，否则工作流中的图片分析等功能不可用。
> 是否安装？
> - 安装 → 继续第一步
> - 跳过 → 不配置 MiMo，**必须告知**"⚠️ 已跳过多模态配置，图片/音频/视频理解功能不可用"
