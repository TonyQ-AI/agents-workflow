...
1. **检测进度跟踪文件** — 检查 `docs/superpowers/plans/` 下是否存在 `task_plan.md`
   - 如果存在：说明之前执行中断，读取 `task_plan.md`、`findings.md`、`progress.md`
   - 向用户报告当前进度，询问是否续做还是重新开始
   - 如果续做：从断点处继续，加载 `findings.md` 中记录的问题
   - 如果重新开始：备份旧文件（加 `.bak` 后缀），从计划文件重新加载
2. 读取计划文件
...
1. **标记为进行中** — 更新 TodoWrite；同时更新 `task_plan.md`（标记当前任务为进行中）、`progress.md`（更新断点）
2. **理解目标** — 重读任务描述，明确完成标准
3. **执行实现** — 严格按照计划步骤执行（计划已有小步骤）
4. **运行验证** — 按要求运行测试或检查
5. **记录发现** — 如果遇到计划未覆盖的情况、做出偏离决策、或发现值得记录的问题，写入 `findings.md`
6. **提交变更** — 每完成一个任务提交一次，commit message 引用任务编号
7. **标记为已完成** — 更新 TodoWrite；同时更新 `task_plan.md`（勾选任务和步骤）、`progress.md`（更新已完成数）

**每个任务的节奏：**
```
--- 任务 2/5：添加用户验证 ---
[更新 progress.md -> 断点: 任务2/步骤1]
[更新 task_plan.md -> 标记任务2为进行中]
...
[更新 task_plan.md -> 勾选任务2全部步骤]
[更新 progress.md -> 已完成: 2/N, 断点: 任务3/步骤1]
--- 任务 2/5 完成 ---
```
...
如果当前会话检测到已存在的 `task_plan.md`，说明之前执行中断：

1. 读取 `task_plan.md` — 查看已勾选的任务和步骤
2. 读取 `progress.md` — 查看断点位置和完成数
3. 读取 `findings.md` — 查看之前记录的发现和决策
4. 向用户报告："检测到之前的执行进度。已完成 X/Y 个任务，断点在任务 N/步骤 M。是否续做？"
5. 如果用户确认续做：从断点处继续，不再重新审查已通过的任务
6. 如果用户选择重来：备份旧文件为 `.bak`，从头执行
